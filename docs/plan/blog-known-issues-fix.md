...
## 问题 1：非 mahiru 的 ADMIN 账户无法使用 Blog Admin ✅ 已完成
...
## 问题 3：置顶功能仅在 Blog Admin，展示端未体现 ✅ 已完成
...
2. **问题 4（回收站）** ✅ 已完成：影响最大，修复最明确
3. **问题 5（点赞账号划分）** ✅ 已完成：数据一致性问题
4. **问题 1（ADMIN 权限）** ✅ 已完成：前端代码已验证正确，AdminGuard 和 load() 逻辑无问题
5. **问题 2（文件上传）** ✅ 已完成：前端代码已验证正确，需运行时验证
6. **问题 3（置顶展示）** ✅ 已完成：体验优化，PostCard 边框高亮 + 归档页左侧蓝色标记
